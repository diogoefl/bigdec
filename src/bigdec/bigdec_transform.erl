...
%% @TODO round(bigdec:bigdec()) -> bigdec:bigdec().
%% @TODO round(bigdec:bigdec(), bigdec:rounding_mode()) -> bigdec:bigdec().
%% @TODO round(bigdec:bigdec(), bigdec:rounding_mode(), integer()) -> bigdec:bigdec().
%% @TODO change_exp(bigdec:bigdec(), integer()) -> bigdec:bigdec().
%% @TODO change_exp(bigdec:bigdec(), integer(), bigdec:rounding_mode()) -> bigdec:bigdec().
%% @TODO rescale_by(bigdec:bigdec(), integer()) -> bigdec:bigdec().
%% @TODO incr_exp(bigdec:bigdec()) -> bigdec:bigdec().
%% @TODO decr_exp(bigdec:bigdec()) -> bigdec:bigdec().

%%---------------------------------------------------------------------------------------------------------------------
%% @doc Negate bigdec changing its sign from 0 to 1 or vice-versa.
%%
%% If sign defined in bigdec is invalid (different from 0 or 1), we return a correction bigdec defining sign as 0
%% (positive of zero).
%% @end
%%---------------------------------------------------------------------------------------------------------------------
...
%%
%% If there is any trailing zero, returns a new bigdec with stripped zeros from the right side ov value and adjusted
%% exp value. If no trailing zeros are found in bigdec, the same bigdec is returned.
%% @end
%%---------------------------------------------------------------------------------------------------------------------
...
strip_zeros(Num = #bigdec{value = Value, exp = Exp}) ->
  case bigdec_analysis:has_trailing_zeros(Num) of
    {false,     0} -> Num;
    {true, Amount} -> Num#bigdec{value = Value div (bigdec_common:hlp_pow(10, Amount)), exp = Exp - Amount}
  end.

...
%%%
%%% Defines common utility functions and helpers to be used internally on the library. These functions are not supposed
%%% to be used outside of bigdec app.
%%% @author diogoefl
...
%%%
%%% This section provides utility functions for common helper patterns that does not have direct connection with
%%% #bigdec{}.
%%%====================================================================================================================
...
%%
%% Execute exponentiation of numbers, using recursive call of erlang pure ints, but it does not do fractional exponent,
%% or count for negative exponent. At these cases it returns error.
%% @end
%%---------------------------------------------------------------------------------------------------------------------
-spec hlp_pow(Number :: integer(), Exponent :: non_neg_integer()) -> Result :: integer().
hlp_pow(Number, Exponent) when is_integer(Number), is_integer(Exponent), Exponent >= 0 ->
...
%%
%% Based on amount of required leading zeros generates string prepending zeros to original string and returns the new
%% string.
%% @end
%%---------------------------------------------------------------------------------------------------------------------
-spec hlp_prepend_zeros(NumberString :: string(), LeadingZeros :: integer()) -> ResultString :: string().
hlp_prepend_zeros(IntString, LeadingZeros) when LeadingZeros > 0 ->
  lists:flatten(lists:duplicate(LeadingZeros, "0")) ++ IntString;
...
%% @end
%%---------------------------------------------------------------------------------------------------------------------
-spec hlp_apply_round(RoundingMode :: bigdec:rounding_mode(),
                      Sign         :: 0 | 1,
                      Digit        :: non_neg_integer(),
                      NextDigit    :: non_neg_integer()) -> Result :: non_neg_integer().
...
hlp_apply_round(round_half_even, S, Digit, Next) when Next rem 2 == 0 -> hlp_apply_round(round_half_up,
                                                                                         S, Digit, Next);
hlp_apply_round(round_half_even, S, Digit, Next)                      -> hlp_apply_round(round_half_down,
                                                                                         S, Digit, Next).
